...
def opt_dist(sequence, D):
    ...
    while k + D - 1 < len(sequence):
        ones_before = 0
        if k > 0:
            ones_before = prefix_sum[k-1]
        ones_after = prefix_sum[len(sequence) - 1] - prefix_sum[k + D - 1]
        ones_in_range = prefix_sum[k + D - 1] - ones_before
...
def mark_pixel(iter_):
    ...
    if iter_ % 30 == 0:
        ...
        for j in range(len(rows[random_row])):
            differences_before = opt_dist(columns[j], column_values[j]) + opt_dist(rows[random_row], row_values[random_row])
            change(random_row, j)
            differences = opt_dist(columns[j], column_values[j])  + opt_dist(rows[random_row], row_values[random_row])
