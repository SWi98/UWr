...
Zadanie 11
```{r}
xs = data.frame(time = seq(0, 10, .01))
vals = exp(predict(model, xs, interval = "prediction")[, 1])
lwr = exp(predict(model, xs, interval = "prediction")[, 2])
upr = exp(predict(model, xs, interval = "prediction")[, 3])

plotting_data = data.frame(vals, lwr, upr, xs)

ggplot(plotting_data) + geom_line(aes(time, upr), col = "red") + 
  geom_line(aes(time, lwr), col = "red") + geom_line(aes(time, vals)) + 
  geom_point(data=old_data, aes(time, concentration), shape=1, size=1) + 
  ylab("concentration")
```
```{r}
